			...
			vk.callFunction({
				...
				success: (res) => {
					...
					if (dateType === 'day' || dateType === 'yesterday') {
						...
						const { yesterdayStart, todayStart } = vk.pubfn.getCommonTime()
						if (dateType === 'yesterday') {
							day = vk.pubfn.getDateInfo(yesterdayStart).day
						}
						let tomorrowDay = vk.pubfn.getDateInfo(
							vk.pubfn.getDayOffsetStartAndEnd(1, dateType === 'day' ? todayStart : yesterdayStart)
								.startTime
						).day
						if (data[tomorrowDay]) {
							data[day].list.push(...data[tomorrowDay].list)
						}
						let allData = {
